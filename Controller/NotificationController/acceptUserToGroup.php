...
if(isset($_SESSION['username'])){
    ...
    //insert into group participants
    $stmt = $db->prepare("insert into groupparticipants(userID,groupID) values(?,?)");
    $stmt->bind_param("ii", $userId, $groupId);
    $stmt->execute();

    //insert into access for the group
    $stmt = $db->prepare("insert into accgroup(userID,access,groupID) values(?,1,?)");
    $stmt->bind_param("ii", $userId, $groupId);
    $stmt->execute();

    $stmt = $db->prepare("delete from grouprequest where userID = ? and groupID = ?");
    $stmt->bind_param("ii", $userId, $groupId);
    $stmt->execute();
